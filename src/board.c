...
/*   Updated: 2019/04/01 21:10:07 by adefonta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		board_init(t_puzzle *puzzle)
{
    ...
    if (puzzle->input == 0)
    {
		if (!(puzzle->queue = state_create(puzzle->board_count, puzzle->board_size)))
		{
			free_all(puzzle);
			return (KO);
		}
		board_copy(puzzle->queue, puzzle->goal);
        randomize(puzzle->queue);
    }
	return (OK);
}
